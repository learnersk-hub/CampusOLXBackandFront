...
import { Search, PlusCircle, User, LogOut, LayoutDashboard, ShoppingBag, ShieldCheck } from 'lucide-react';
    ...
    <nav className="sticky top-0 z-50 w-full border-b bg-white/70 dark:bg-gray-950/70 backdrop-blur-xl supports-[backdrop-filter]:bg-white/50 dark:supports-[backdrop-filter]:bg-gray-950/50">
      <div className="container flex h-16 items-center justify-between px-4 mx-auto max-w-7xl">
        <div className="flex items-center gap-8">
          <Link href="/" className="flex items-center space-x-2 transition-transform hover:scale-[1.02] active:scale-[0.98]">
            <div className="h-8 w-8 rounded-lg bg-gradient-to-br from-indigo-600 to-purple-600 flex items-center justify-center shadow-lg shadow-indigo-200 dark:shadow-none">
              <span className="text-white font-black text-sm">C</span>
            </div>
            <span className="text-xl font-black tracking-tight bg-gradient-to-r from-indigo-600 to-purple-600 bg-clip-text text-transparent hidden sm:block">
              CampusOLX
            </span>
          </Link>
          <form onSubmit={handleSearch} className="hidden lg:flex relative w-[400px]">
        ...
        <div className="flex items-center gap-3">
          ...
          {isAuthenticated ? (
            ...
            <div className="flex items-center gap-2">
              <Link href="/login">
                <Button variant="ghost" className="rounded-xl font-bold px-5">Login</Button>
              </Link>
              <Link href="/signup">
                <Button className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold px-5 shadow-lg shadow-indigo-200 dark:shadow-none transition-all active:scale-95">
                  Get Started
                </Button>
              </Link>
            </div>
